...
public class MyStateMachinePersist<S, E> implements StateMachinePersist<S, E, StateMachineInfo> {
	...
	@Override
	public void write(StateMachineContext<S,E> context, StateMachineInfo stat) {
		byte[] data = serialize(context);
		String content = new String(Base64.getEncoder().encode(data));
		stat.setContent(content);
		stateMachineInfoRepository.save(stat);
	}

	@Override
	public StateMachineContext<S, E> read(StateMachineInfo stat) throws Exception {
		return deserialize(Base64.getDecoder().decode(stat.getContent().getBytes()));
	}
	...
	public StateMachineInfoRepository getStateMachineInfoRepository() {
		return stateMachineInfoRepository;
	}

	public void setStateMachineInfoRepository(
			StateMachineInfoRepository stateMachineInfoRepository) {
		this.stateMachineInfoRepository = stateMachineInfoRepository;
	}
